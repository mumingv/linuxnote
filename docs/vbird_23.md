...
|发行版             |RedHat/Fedora/CentOS/SuSE  |Debian/Utuntu      |
|-------------------|---------------------------|-------------------|
|软件管理机制       |RPM                        |DPKG               |
...
补充说明：
1. CentOS是“社区维护的企业版”，因为其实社区根据RHEL发布的源码（SRPM）重新编译发布的产物；
2. 可以通过SRPM内部的编译参数来学习CentOS是如何编译一个程序的。


### 什么是i386、i586、i686、noarch、x86_64

示例：对于`rp-pppoe-3.1-5.i386.rpm`
...
3. 容器的使用时比较灵活的，一个软件可以放置在不同的容器当中。


## RPM软件管理程序：rpm

RPM数据库文件所在的目录为：`/var/lib/rpm`，当软件安装完毕后，该软件相关的信息都会被写入目录下的数据库文件当中。

安装软件涉及到的几个目录：
1. `/etc`：配置文件；
2. `/usr/bin`：可执行文件；
3. `/usr/lib`：程序使用的动态函数库；
4. `/usr/share/doc`：软件使用手册与帮助文档；
5. `/usr/share/man`：一些man手册页；


### RPM安装（`-i --install`）

重要说明：安装软件时root用户的工作，所以使用root的身份才能运行rpm命令。

基本语法：
1. 安装本地软件：`rpm -ivh software.x86_64.rpm`
2. 安装网络软件：`rpm -ivh http://website.com/path/software.x86_64.rpm`
...
7. `--oldpackage`：如果已经安装过该软件，并且当前安装的软件版本比已经安装的软件版本还要老，也允许替换成老的
8. `--force`：--replacepkgs, --replacefiles 和 --oldpackage 这三者的合体
9. `--test`：检查安装该软件是否存在依赖性问题或者冲突，不会实际安装软件
10. `--justdb`：更新软件在rpm数据库中的信息，不会更新文件系统，一般在rpm数据库损坏后者出现异常时使用
